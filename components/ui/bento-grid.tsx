    ...
    >
      <div className={cn("h-full", id === 6 && "flex justify-center")}>
        ...
        >
          ...
          {/* Tech Stack – 2 Baris */}
      {id === 3 && (
            <div className="flex flex-col gap-4 lg:gap-6  mt-4 z-0 -right-3 lg:-right-2">
              {/* Baris pertama: tech stack 1 */}
              <div className="flex justify-center gap-4 w-full lg:gap-8">
                {techStack.stack1.map((item) => (
                  <span
                    key={item}     
                    className="rounded-lg bg-[#10132e] px-2 py-2 text-center text-sm opacity-50 lg:text-base lg:opacity-100"
                  >
                    {item}
                  </span>
                ))}
